...
if test_bet:
    ...
    agent = DQNAgent(env=env_1, epsilon=0, loaded_model="models/128x64_noexp_2")
    betting_env = BlackjackWithBetEnv(blackjackEnv=env_1, playerAgent=agent, max_bet=10_000)
    ...
    if test_dqn:
        from agents.dqn import DQNAgent
        agent = DQNAgent(env=env_1, epsilon=0, loaded_model="models/128x64x32_val=-0.035_1702857898.model")
        test_agent(agent=agent, blackjack_env=env_2, num_turns=num_turns)
    if all:
        from agents.baseline import BaselineAgent
        from agents.dqn import DQNAgent
        ...
        dqn_agent = DQNAgent(env=env_1, epsilon=0, loaded_model="models/128x64x32_val=-0.035_1702857898.model")
